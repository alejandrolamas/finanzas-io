  ...
  return (
    <div className="space-y-3">
      <div>
        ...
        <p>{(transaction.category as any)?.name || "N/A"}</p>
      </div>
      <div>
        <p className="text-sm text-muted-foreground">Cuenta</p>
        <p>{(transaction.account as any)?.name || "N/A"}</p>
      </div>
      <div>
        <p className="text-sm text-muted-foreground">Naturaleza</p>
        <p>{transaction.nature}</p>
      </div>
    </div>
  )
}

export function TransactionList({ initialTransactions }: { initialTransactions: ITransaction[] }) {
  ...
  const handleTransactionAdded = (newTransaction: ITransaction) => {
    // Add the new transaction to the beginning of the list (newest first)
    setTransactions([newTransaction, ...transactions])
  }

  const handleTransactionUpdated = (updatedTransaction: ITransaction) => {
